...
class EV2REvaluator:
    ...
    def __init__(self, properties=None, model="meta-llama/Meta-Llama-3.1-8B-Instruct"):
        ...
        # Replace API client with local model
        self.model = LLM(
            model=model,
    ...
    def query_local_model(self, prompt):
        try:
            ...
            response_llm = outputs[0].outputs[0].text.strip()
...
def compute(solution_file, submission_file, model="meta-llama/Meta-Llama-3.1-8B-Instruct"):
    ...
    # Q only
    Q_evidence_score, Q_score_list = scorer.evaluate_questions_only(public_submission_df[target_cols],
                                                                    public_solution_df[target_cols])
    # Q + A
    QA_evidence_score, QA_score_list = scorer.evaluate_questions_and_answers(public_submission_df[target_cols],
                                                                             public_solution_df[target_cols])
    averitec_scores, averitec_score_list = scorer.evaluate_averitec_score(public_submission_df[target_cols],
                                                                          public_solution_df[target_cols])

    # EV2R Score
    EV2R_scorer = EV2REvaluator(properties, model)
...
def main():
    ...
    #
    compute(args.label_file, args.prediction_file, args.model)
