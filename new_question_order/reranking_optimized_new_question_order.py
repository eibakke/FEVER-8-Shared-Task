...
def main(args):
    ...
    with open(args.json_output, "w", encoding="utf-8") as output_json:
        ...
        for idx, example in enumerate(target_examples):
            if idx in files_to_process:
                ...
                # Extract passages
                hyde_field = (example.get('hypo_passage') or
                              example.get('hypo_fc_docs') or [])
                hyde_docs = hyde_field if isinstance(hyde_field, list) else [hyde_field]

                # NEW: Extract questions
                questions = []
                # Try different potential field names for questions
                if "questions" in example:
                    questions = example["questions"]
                elif "fcq_pairs" in example and example["fcq_pairs"]:
                    for pair in example["fcq_pairs"]:
                        if "questions" in pair:
                            questions.extend(pair["questions"])
                ...
                # Add passages to query
                for doc in hyde_docs:
                    if isinstance(doc, str) and doc.strip():
                        query.append(get_detailed_instruct(task, doc))

                # NEW: Add questions to query for better reranking
                for question in questions:
                    if isinstance(question, str) and question.strip():
                        query.append(get_detailed_instruct(task, question))

                query_length = len(query)
                print(f"Using {query_length} query components ({len(questions)} questions)")
                ...
                try:
                    ...
                    # Include questions in the output for downstream tasks
                    json_data = {
                        "claim_id": example['claim_id'],
                        "claim": claim,
                        f"top_{args.top_k}": top_k_sentences_urls,
                        "questions": questions  # Save questions for later steps
                    }
    ...
    # Fix the timing calculation bug
