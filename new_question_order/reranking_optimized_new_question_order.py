...
def main(args):
    ...
    with open(args.json_output, "w", encoding="utf-8") as output_json:
        ...
        for idx, example in enumerate(target_examples):
            if idx in files_to_process:
                ...
                #Change in with new order format, accepts both old and new
                claim = example['claim']
                hyde_field = (example.get('hypo_passage') or #New
                              example.get('hypo_fc_docs') or []) #Old

                hyde_docs  = hyde_field if isinstance(hyde_field, list) else [hyde_field]

                query = [get_detailed_instruct(task, claim)] + [
                    get_detailed_instruct(task, doc)
    ...
    avg_time = total_time / total_time
