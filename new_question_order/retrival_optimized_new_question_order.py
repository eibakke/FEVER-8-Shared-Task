...
def process_single_example(idx, example, args, result_queue, counter, lock):
    try:
        ...
        #NEW: Handeling of old and new way of storing data 
        hyde = example.get("hypo_passage") or example.get("hypo_fc_docs") or []
        hyde_docs = hyde if isinstance(hyde, list) else [hyde]
        query = example["claim"] + " " + " ".join(hyde_docs)
