...
def group_evidence_by_type(evidence: List[Dict[str, Any]]) -> Dict[str, List[Dict[str, Any]]]:
    """Group evidence by fact-checking type"""
    result = {}
    for item in evidence:
        fc_type = item.get("fc_type", "neutral")
        if fc_type not in result:
            result[fc_type] = []

        # Create a copy without the fc_type field to match original format
        evidence_item = {
            "question": item["question"],
            "answer": item["answer"],
            "url": item["url"]
        }
        result[fc_type].append(evidence_item)

    return result


def prepare_prompts(examples: List[Dict], tokenizer, model_name: str) -> List[str]:
    """Prepare prompts for batch processing with multiple fact-checking perspectives."""
    base_prompt = """Your task is to predict the verdict of a claim based on provided question-answer pairs from multiple fact-checking perspectives.
Choose from the labels: 'Supported', 'Refuted', 'Not Enough Evidence', 'Conflicting Evidence/Cherrypicking'.
Disregard irrelevant question-answer pairs when assessing the claim.
Different perspectives (positive, negative, objective) have been used to gather evidence. Consider all perspectives equally.
Justify your decision step by step using the provided evidence and select the appropriate label."""

    prepared_inputs = []
    for example in examples:
        # Group evidence by fact-checking type
        evidence_by_type = group_evidence_by_type(example["evidence"])

        # Build the prompt with evidence from each perspective
        example_prompt = base_prompt + "\n\nClaim: " + example["claim"] + "\n\n"

        for fc_type, evidence_list in evidence_by_type.items():
            example_prompt += f"Evidence from {fc_type.capitalize()} Perspective:\n"
            for i, qa in enumerate(evidence_list):
                example_prompt += f"Q{i + 1}: {qa['question']}\nA{i + 1}: {qa['answer']}\n\n"

        example_prompt = example_prompt.strip()
        example["input_str"] = example_prompt

        if "OLMo" in model_name:
            input_ids = [example["input_str"]]
        else:
...
def main(args):
    ...
    try:
        ...
        try:
            ...
            print("Error: Could not parse the input file as JSON")
            return
    ...
    with torch.no_grad():
        for batch_idx in range(0, len(examples), batch_size):
            ...
            for example, output in zip(current_batch, outputs):
                ...
                # Prepare the common evidence format for compatibility with evaluation
                flat_evidence = []
...
if __name__ == "__main__":
    ...
    parser.add_argument("-i", "--target_data", default="data_store/merged_qa.json")
    parser.add_argument("-o", "--output_file", default="data_store/veracity_prediction.json")
