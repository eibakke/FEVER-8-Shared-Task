...
def prepare_prompts(examples: List[Dict], tokenizer, model_name: str) -> List[str]:
    ...
    for example in examples:
        if "evidence" in example:                       # old format (QA pairs)
            ev_list = example["evidence"]
        else:                                           # new format from ranking.py
            top_field = next(k for k in example if k.startswith("top_"))
            ev_list = example[top_field]

        ev_lines = []
        for i, ev in enumerate(ev_list):
            if "question" in ev and "answer" in ev:     # QA pair
                ev_lines.append(f"Q{i+1}: {ev['question']}\nA{i+1}: {ev['answer']}")
            else:                                       # plain sentence
                ev_lines.append(f"Evidence{i+1}: {ev.get('sentence', str(ev))}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("-i", "--target_data", default="data_store/dev_reranking_top_k.json")
