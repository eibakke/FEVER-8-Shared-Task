...
def process_single_example(idx, example, args, result_queue, counter, lock):
    try:
        ...
        result = {
            "claim_id": idx,
            "claim": example["claim"],
            f"top_{args.top_k}":  [{"sentence": s, "url": u} for s, u in zip(top_k_sentences, top_k_urls)],
            "questions": questions
        }
        ...
        print(f"[WORKER {idx}] Uncaught exception:\n", file=sys.stderr)
        traceback.print_exc() 
...
def main(args):
    logger = multiprocessing.log_to_stderr()   # returns a standard `logging.Logger`
    ...
    with Manager() as manager:
        ...
        with Pool(num_workers) as pool:
            ...
            #for idx, result in pool.imap_unordered(process_func, examples_to_process, chunksize=16):
            #    result_queue.put((idx, result))
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--target_data",
        type=str,
        default="data_store/dev_fcq.json", #hyde_fc.json",
