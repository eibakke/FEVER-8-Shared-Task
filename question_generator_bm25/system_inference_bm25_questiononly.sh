...
while [[ $# -gt 0 ]]; do
  case $1 in
    ...
    *)
      ...
      echo "Usage: /system_inference_embeddings_chroma.sh [--num-examples=N] [--system=NAME] [--split=SPLIT] [--base-dir=DIR] [--resume-step=STEP]"
      exit 1
      ;;
  esac
done
...
if [ $RESUME_STEP -le 1 ]; then
    ...
    python "${CODE_PATH}/../embeddings/question_generator.py" \
...
if [ $RESUME_STEP -le 2 ]; then
    ...
    python "${CODE_PATH}/retriever_bm25_question_only.py" \
...
if [ $RESUME_STEP -le 3 ]; then
    ...
    python "${CODE_PATH}/../embeddings/reranking_optimized_embeddings_questiononly.py" \
...
if [ $RESUME_STEP -le 5 ]; then
    ...
    python "${CODE_PATH}/veracity_prediction_bm25_questiononly.py" \
        --target_data "${DATA_STORE}/${SYSTEM_NAME}/${SPLIT}_reranking_top_k.json" \
