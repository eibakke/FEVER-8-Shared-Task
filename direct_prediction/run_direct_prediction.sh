...
# Get SYSTEM_NAME from direct_system_inference.sh environment
eval $(grep '^SYSTEM_NAME=' direct_system_inference.sh)
...
# Pass all command-line arguments to direct_system_inference.sh
run_script "Direct Prediction Execution" ./direct_system_inference.sh "$@" || {
    log_timing "Error: Direct prediction execution failed"
    exit 1
}
